...
@section('content')
  <section class="section">
    <div class="row">
      <div class="col d-flex justify-content-end mb-3">
        <button type="button" data-bs-toggle="modal" data-bs-target="#addDependent" class="btn btn-success">
              ...
              <table class="table table-striped" id="table">
                <thead>
                  <tr>
                    ...
                    <th scope="col">{{ __('dependants.relationship') }}</th>
                    <th scope="col">{{ __('dependants.gender') }}</th>
                    <th scope="col">{{ __('dependants.insurance') }}</th>
                ...
                <tbody>
                  ...
                  @foreach ($dependents as $dependent)
                    <tr>
                      ...
                      <td>{{ app()->getLocale() == 'ar' ? $dependent->gender?->gender_ar : $dependent->gender?->gender_en }}
                      </td>
                      <td>
                        @if ($dependent->insurance)
                          <svg fill="#006400" width="15px" height="15px" viewBox="0 0 1920 1920"
                      ...
                      <td>
                        <button type="button" data-bs-toggle="modal" data-bs-target="#editDependent"
                          data-id="{{ $dependent->id }}" data-iden="{{ $dependent->identification }}"
                          data-name="{{ $dependent->name }}" data-dob="{{ $dependent->date_of_birth }}"
                          data-rel="{{ $dependent->relationship_id }}" data-gen="{{ $dependent->gender_id }}"
                          class="btn btn-warning btn-sm py-0" data-bs-placement="top" title="{{ __('global.edit') }}">
                          <i class="bi bi-pencil-square"></i>
                        </button>
                        <button type="button" class="btn btn-danger btn-sm py-0" id="btn" data-id="{{ $dependent->id }}"
                          data-bs-toggle="modal" data-bs-target="#delteConfirmation" data-bs-placement="top"
              ...
              <div class="col">
                ...
                <select class="form-select" id="relationship_id_edit" name="relationship_id" style="width:100%">
                  ...
                  @foreach ($relationships as $relationship)
                    <option value="{{ $relationship->id }}" @selected($relationship->id == old('relationship->id'))>
                      {{  $relationship->{'relationship' . session('_lang')} }}</option>
                  @endforeach
                </select>
              </div>
            </div>
            ...
            <div class="row">
              <div class="col">
                ...
                <select class="form-select" id="relationship" name="relationship_id" style="width:100%">
                  <option selected disabled>{{ __('global.select') }}</option>
                  @foreach ($relationships as $relationship)
                    <option value="{{ $relationship->id }}" @selected($relationship->id == old('relationship_id'))>
                      {{  $relationship->{'relationship' . session('_lang')} }}</option>
                  @endforeach
                </select>
              </div>
  ...
  <!-- Delete Modal -->
  <div class="modal fade" id="delteConfirmation" tabindex="-1" aria-labelledby="delteConfirmationLabel"
    aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
