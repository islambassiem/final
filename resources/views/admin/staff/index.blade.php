    ...
    <section class="section">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="status" class="form-control js-example-basic-single">
                                    ...
                                    <option value="1" {{ request()->get('status') == '1' ? 'selected' : '' }}>
                                        {{ __('admin/staff.active') }}</option>
                                    <option value="0" {{ request()->get('status') == '0' ? 'selected' : '' }}>
                                        {{ __('admin/staff.resigned') }}</option>
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3">
                                ...
                                <select name="gender" class="form-control">
                                    ...
                                    @foreach ($genders as $gender)
                                        <option value="{{ $gender->id }}"
                                            {{ request()->get('gender') == $gender->id ? 'selected' : '' }}>
                                            {{ $gender->{'gender' . session('_lang')} }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                ...
                                <select name="nationality" class="form-control">
                                    ...
                                    @foreach ($nationalities as $nationality)
                                        <option value="{{ $nationality->id }}"
                                            ...
                                            {{ $nationality->{'country' . session('_lang')} }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="mb-3">
                                ...
                                <select name="saudization" class="form-control js-example-basic-single">
                                    <option value="" selected>{{ __('admin/staff.selectAll') }}</option>
                                    <option value="1" {{ request()->get('saudization') == '1' ? 'selected' : '' }}>
                                        {{ __('admin/staff.saudi') }}</option>
                                    <option value="0" {{ request()->get('saudization') == '0' ? 'selected' : '' }}>
                                        {{ __('admin/staff.expat') }}</option>
                                </select>
                            </div>
                        </div>
                    </div>
                                ...
                                <select name="section[]" class="form-control" multiple="">
                                    @foreach ($sections as $section)
                                        <option value="{{ $section->id }}"
                                            ...
                                            {{ $section->{'section' . session('_lang')} }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                ...
                                <select name="category[]" class="form-control" multiple="multiple">
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            ...
                                            {{ $category->{'category' . session('_lang')} }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="row">
                                ...
                                <select name="sponsorship[]" class="form-control" multiple="multiple">
                                    @foreach ($sponsorships as $sponsorship)
                                        <option value="{{ $sponsorship->id }}"
                                            {{ request()->get('sponsorship') == $sponsorship->id ? 'selected' : '' }}>
                                            {{ $sponsorship->{'sponsorship' . session('_lang')} }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="col-md-5">
                            ...
                            <input type="text" name="search" value="{{ request()->search }}" id="search"
                                placeholder="{{ __('admin/staff.search') }}" class="form-control">
                        </div>
                    </div>
                    <div class="row">
                        <div class="d-flex justify-content-between">
                            <div>
                                <a href="{{ route('admin.staff.download') }}" class="btn btn-success d-flex">
                                    <i class="bi bi-file-earmark-arrow-down-fill me-1"></i>
                                    {{ __('admin/staff.downloadAll') }}
                                </a>
                            </div>
                            <div class="d-flex justify-content-end">
                                ...
                                <button type="submit" class="btn btn-secondary mx-2 d-flex" id="download"><i
                                    ...
                                    {{ __('admin/staff.download') }}</button>
                            </div>
                        </div>
                    </div>
                </form>
        ...
        <div class="card card-body">
            <div class="card-title">
                        ...
                        @foreach ($staff as $member)
                            <tr>
                                ...
                                <td @if (!$member->active) class="text-danger" @endif>
                                    {{ session('_lang') == '_ar' ? $member->getFullArabicNameAttribute : $member->getFullEnglishNameAttribute }}
                                </td>
                                <td @if (!$member->active) class="text-danger" @endif>
                                    {{ $member->iqama($member->id)->document_id }}</td>
                                <td @if (!$member->active) class="text-danger" @endif>
                                    {{ $member->latestSalary($member->id) }}</td>
                                <td @if (!$member->active) class="text-danger" @endif>
                                    {{ $member->extension($member->id) }}</td>
                                <td @if (!$member->active) class="text-danger" @endif>
                                    {{ $member->mobile($member->id) }}</td>
                                ...
                                <td @if (!$member->active) class="text-danger" @endif>
                                    <a href="{{ route('admin.employee', $member) }}" class="btn btn-primary btn-sm py-0"
                                        ...
                                        data-bs-title="{{ __('global.view') }}">
                                        <i class="bi bi-person-fill-gear"></i></a>
                                    @if ($member->active)
                                        <button type="button" class="btn btn-danger btn-sm py-0" data-bs-toggle="modal"
                                            data-bs-target="#leaverModal" data-bs-placement="top"
                                            title="{{ __('global.resingnation') }}" data-id="{{ $member->id }}">
                ...
                <div class="d-flex justify-content-between">
                    <div>{{ $staff->links() }}</div>
                    <div class="d-flex justify-content-between align-items-center">{{ __('admin/staff.total') }} {{ $staff->total() }}</div>
                </div>
            </div>
        </div>
    </section>
